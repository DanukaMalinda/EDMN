from data.superconductor_binary import prep
import numpy as np
...
def load_data():
    ...
    # dataset
    X_n = dataset.drop(columns=['critical_temp']).values.astype(np.float32)
    y_n = dataset['critical_temp'].values.astype(np.int64)
