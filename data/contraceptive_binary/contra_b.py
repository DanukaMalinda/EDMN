from data.contraceptive_binary import prep
import numpy as np
...
def load_data():
    ...
    # dataset
    X_n = dataset.drop(columns=['Contraceptive']).values.astype(np.float32)
    y_n = dataset['Contraceptive'].values.astype(np.int64)
