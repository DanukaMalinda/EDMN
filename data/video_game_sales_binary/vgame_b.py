from data.video_game_sales_binary import prep
import numpy as np
...
def load_data():
    ...
    # dataset
    X_n = dataset.drop(columns=['Critic_Score']).values.astype(np.float32)
    y_n = dataset['Critic_Score'].values.astype(np.int64)
