from data.human_activity_recognition import prep
import numpy as np
...
def load_data():
    ...
    # dataset
    X_n = dataset.drop(columns=['Activity', 'subject']).values.astype(np.float32)
    y_n = dataset['Activity'].values.astype(np.int64)
