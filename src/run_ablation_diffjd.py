"""Ablation study for EDMN -- discrete neighborhood search vs. differentiable JD +
gradient descent, run side by side on the identical trained model.

Copied from run_ablation.py (same 5-variant scaffold, same data splits, same
warm-start behavior) so nothing in run_ablation.py, edm.py, distance_metrics.py or
enn.py needs to change. The only difference from run_ablation.py: for every
(config, variant), after training the model once, the EDM estimation step is run
TWICE on the exact same histograms/initial solutions --

  1. edm.get_estimation           -- the existing discrete neighborhood search
  2. edm_diffjd.get_estimation_diffjd -- Adam gradient descent on a differentiable
                                         Jensen Divergence, started from the same
                                         initial solution
...
# Arguments
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="Ablation study for EDMN -- discrete vs diffjd search")
parser.add_argument('--dataset',          type=str,   required=True)
parser.add_argument('--distance_metric',  type=str,   default='JD',
                    help='edm_diffjd only implements a differentiable JD; must be JD')
...
parser.add_argument('--smoothing',      type=float, default=0.025,
                    help='label smoothing used in variants 2, 5 & 6 during training')
...
if args.distance_metric != 'JD':
    raise ValueError("edm_diffjd only implements a differentiable JD; "
...
# ---------------------------------------------------------------------------
# Data split utilities  (identical to run_ablation.py / run_enn.py)
# ---------------------------------------------------------------------------
...
def run_edm_estimation_compare(model, train_loader, test_loader, num_classes, neighborhood_steps):
    ...
    the existing discrete neighborhood search (edm.get_estimation, unchanged), once
    with the differentiable JD + gradient descent search (edm_diffjd), both started
    from the same two candidate initial solutions edm.get_init_solution produces.
    ...
    starts = [initial_solution, init_estimate_pred]

    # -- discrete neighborhood search (existing edm.get_estimation, unchanged) --
    d_disc, fe_disc = np.inf, None
    t0 = time.perf_counter()
    for ini_sol in starts:
        fe, distance = edm.get_estimation(
            train_hist_dict, test_hist_dict, num_classes,
            neighborhood_steps, ini_sol, dm)
        if distance < d_disc:
            d_disc, fe_disc = distance, fe
    time_disc = time.perf_counter() - t0

    # -- differentiable JD + Adam gradient descent search --
    d_diffjd, fe_diffjd = np.inf, None
    t0 = time.perf_counter()
    for ini_sol in starts:
        fe, distance = edm_diffjd.get_estimation_diffjd(
            train_hist_dict, test_hist_dict, num_classes, ini_sol,
            steps=search_steps, lr=search_lr)
        if distance < d_diffjd:
            d_diffjd, fe_diffjd = distance, fe
    ...
    return {
        ...
        'disc':   {'mae': edm.AE(act, est_disc, num_classes),
                   'nkld': edm.NKLD(act, est_disc, num_classes),
                   'time': time_disc, 'est': est_disc},
        'diffjd': {'mae': edm.AE(act, est_diffjd, num_classes),
                   'nkld': edm.NKLD(act, est_diffjd, num_classes),
                   'time': time_diffjd, 'est': est_diffjd},
    }


# ---------------------------------------------------------------------------
# Ablation variant definitions  (identical to run_ablation.py)
# ---------------------------------------------------------------------------
ABLATION_VARIANTS = [
    {
...
# ---------------------------------------------------------------------------
# Prevalence tables  (identical to run_ablation.py / run_enn.py)
# ---------------------------------------------------------------------------
...
def print_summary(all_results):
    print(f"\n{SEP}")
    print(f"  ABLATION SUMMARY -- discrete search vs differentiable JD + gradient descent")
...
def main():
    ...
    # Output CSV -- own directory, never touches ../results_ablation/
    out_dir = '../results_ablation_diffjd'
    os.makedirs(out_dir, exist_ok=True)
    csv_path = f'{out_dir}/ablation_{args.dataset}_{dm}_diffjd.csv'
    ...
    for dt_distr, train_distr, test_distr, seed, bins_val in configs:
        ...
        try:
            ...
            for variant in ABLATION_VARIANTS:
                ...
                short = variant['short']
                try:
                    # Full EDMN warm-starts from the Label Smooth model, matching
                    # run_ablation.py's (v1) behavior.
